...
- **3rd language** (user-selectable): controlled by `lang3Code` (persisted to `localStorage`); options defined in `LANG3_OPTIONS` (line 295). Currently: FRE, NEP, SPA, ARA, POR, CHN, VIE, LAO. Changing the selection calls `saveLang3()` (line 552).
...
- Translation uses `claude-sonnet-4-20250514` via `callAnthropicApi()` (line 575). Calls are sequential with a 1-second delay between requests for rate limiting. Translation targets English and the active 3rd language (`lang3Code`).

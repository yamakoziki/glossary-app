...
- **Japanese** (`ja`, always): source language; always present
- **English** (`en`, always): translated; always present as the second language button
- **3rd language** (`lang3`, user-selectable): controlled by `lang3Code` (persisted to `localStorage`); options defined in `LANG3_OPTIONS` (line 297). Currently: FRE, NEP, SPA, ARA, POR, CHN, VIE, LAO. Changing the selection calls `saveLang3()` (line 549).
...
- Translation uses `claude-sonnet-4-20250514` via `callAnthropicApi()` (line 574). Task list is built by `buildTranslationTasks()` (line 601) and executed sequentially by `runTranslation()` (line 632) with a 1-second delay between requests for rate limiting. Translation targets English and the active 3rd language (`lang3Code`).
